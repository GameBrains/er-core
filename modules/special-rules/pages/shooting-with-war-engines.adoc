...
footnote:weapon-ms[{e40k-diff}While {e40k} does mention 'weapon mounts', it does not define the term or use it consistently, and it mixes the concept with another concept that we call 'weapon systems'. We define these terms and concepts so that we can explain certain rules interactions more clearly.]
...
footnote:[{e40k-diff}In {e40k} there are only Front (90°), Left/Right Side (180°) and All Round (360°) 'weapon fire arcs'. We add more options, for more flexible unit design. And we use the more generic term 'weapon arc', to be more inclusive of weapons that may not have a ranged component, such as {ripper-tentacles}.]
