...
== Flyers and {transport-x}

* You can use Flyer units that have the {transport-x} ability to carry valid cargo units from other detachments in your army.
Keep the carried detachment outside the play area along with the Flyer detachment until you are ready to deploy them.
...
* The flyer detachment must have at least one {transport-X} unit available in the Ready state.
...
But note:
* You can attack only where an enemy unit lies within range *and* inside the forward 90° arc of your Flyer unit.
Flyer units do not need line of fire, as they can see over all intervening units and terrain.
...
+
{blank}[IJW reply: Yes, there are. A 60 mm round base (fairly common for stability purposes) makes it impossible to disembark units on 20 mm square or 25 mm round bases, and even strip bases barely squeeze in by a few mm. There are also 115 x 90 mm oval bases that are sometimes used for SM Landing Craft, or bigger Ork Landa conversions. I suggest using the standard disembarkation rules, and add a NOTE saying that landing Flyers can't be placed within 10 cm of enemy units and are Vehicles for terrain purposes.
+
Plus there are players that use scenic bases rather than traditional flight stands, so there is no central column to measure from anyway.]

*  You can only give the disembarking detachment normal orders or Assault orders.
+
...
+
{blank}[IJW reply: I'm not sure it matters, 3rd doesn't really have discrete steps inside a player's part of the Movement phase.]

. After you complete the disembarkation, the transport flyers take off, and all the flyers disengage.
