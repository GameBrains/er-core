...
. In the Start phase of each turn, place five Initiative counters in the mug -- three for the player whose army has the higher Strategy Rating, and two for the player whose army has the lower Strategy Rating.
footnote:[
include::main-rules:partial$start-phase.adoc[]
]
footnote:[If both armies have the same Strategy Rating then, at the start of the game, roll off to decide which army gets the advantage for this game.]
. Where the turn sequence instructs you to "Determine the Initiative", one player must draw a random counter from the mug.
